...
% 全局变量
local_path ='G:\' ;
svn_path = 'thesis_experiment_rxw\datasets\ballet\MSR3DVideo-Ballet\';%原始图像路径
tic;
%% 3D image warping for 100 frames
% initialise parameters
calibParams_Ballet; % read the calibration parameters of “ballet” sequence
% 
% for idx = 0 : 99
...
%        end
%     end
%     FileName = strcat( 'Ides_ballet\Ides_ballet_des',int2str(idx),'.bmp');
%     imwrite(Ides, FileName);
%     FileName = strcat('Ides_ballet\Is_Ballet_m', int2str(idx), 'M');
...
%     if (pPointer > 0)
%         pPointer = pPointer - 1;
%         FileName = strcat('Ides_ballet\Ides_Ballet_des', int2str(pPointer), '.bmp');
%         Ides_other_p = imread(FileName);
%         FileName = strcat('Ides_ballet\Is_Ballet_m', int2str(pPointer), 'M');
%         M_other_p = dlmread( FileName);
%         [Fig6_c, Fig6_c_m] = sdr_copy_single_frame(Fig6_c, Ides_other_p, Fig6_c_m, M_other_p, 110, 768, 1024);
%     end
%     
%     % 用当前帧后面的帧去补
%     if (nPointer < 99)
%         nPointer = nPointer + 1;
%         FileName = strcat('Ides_ballet\Ides_Ballet_des', int2str(nPointer), '.bmp');
%         Ides_other_n = imread(FileName);
%         FileName = strcat('Ides_ballet\Is_Ballet_m', int2str(nPointer), 'M');
%         M_other_n = dlmread(FileName);
%         [Fig6_c, Fig6_c_m] = sdr_copy_single_frame(Fig6_c, Ides_other_n, Fig6_c_m, M_other_n, 110, 768, 1024);
%     end
% end
% FileName = strcat( 'SDR_ballet\Ides_sdr_ballet_',int2str(idx),'.bmp');
% imwrite(Fig6_c, FileName);
% % imshow(uint8(Fig6_c));
% FileName = strcat('SDR_ballet\Ides_sdr_ballet_',int2str(idx),'M');
...
% FileName = strcat('projection_ballet\color', int2str(idx),'.bmp');
% imwrite(uint8(Ides),FileName);
% FileName = strcat('projection_ballet\depth', int2str(idx),'.bmp');
...
%     [Ides,M] = projection_holefilling(Is,M,original_M,IR,D,MinZ, MaxZ, m5_ProjMatrix, m4_ProjMatrix);
    FileName = strcat('projection_holefilling_ballet\Ides_ project-',int2str(idx),'.bmp');
    imwrite(uint8(Ides),FileName);
    ...
    dD = uint8(dD);
    FileName = strcat('projection_holefilling_ballet\Ides_ project-depth',int2str(idx),'.bmp');
    imwrite(uint8(dD),FileName);
%% 后处理-滤波
    Ides=Filter(Ides);
%     imshow(Ides);
...
%     FileName = strcat('filter_ballet\Ides-',int2str(idx),'.bmp');
%     imwrite(uint8(Id),FileName);
% %     figure;
% %     imshow(uint8(Id));
%     FileName = strcat('filter_ballet\Ddes-',int2str(idx),'.bmp');
%     imwrite(uint8(Ddes),FileName);
    
        FileName = strcat('final_Ides\Is_ballet_des',int2str(idx),'.png');
    imwrite(uint8(Id),FileName);
%     figure;
%     imshow(uint8(Id));
    FileName = strcat('final_Ides\Is_ballet_des',int2str(idx),'_mask.png');
    imwrite(uint8(Ddes),FileName);
end
toc;
%% output